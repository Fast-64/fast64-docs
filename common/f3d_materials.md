...
Chroma keying allows you to specify colors as pixel alpha. A combiner of `(input - KeyCenter)*KeyScale + 0` is recommended for use and should be used in the last available cycle. The combiner will use the `input` value put in variable A as the color output, and the alpha out will be the min value from each color channel of `clamp(0, key width - abs(combiner out), 255)`.
In order to use chroma key, `alpha_cvg_sel` must be off in the chosen render mode. When using chroma key, `cvg_x_alpha` will be ignored, making tex edge operations not work.
...
Alpha compare will determine if pixels are written based on passed alpha values from the color combiner. In threshold mode, the pixel is written if the value is >= Blend alpha. In dither, the pixel is written if the alpha is >= a random value. Alpha compare can be used in 1 or 2 cycle and copy mode.
